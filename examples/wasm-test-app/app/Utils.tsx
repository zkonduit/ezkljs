...
async function convertFilesToFilesSer<T extends FileMapping>(files: T): Promise<FileSerMapping> {
...
export async function handleGenPkButton<T extends FileMapping>(files: T): Promise<Uint8Array> {
    const result = await convertFilesToFilesSer(files)
    return genPk(result["model"], result["srs"], result["circuitSettings"]);
}

export async function handleGenProofButton<T extends FileMapping>(files: T): Promise<Uint8Array> {
    const result = await convertFilesToFilesSer(files)
    return prove(result["data"], result["pk"], result["model"], result["circuitSettings"], result["srs"]);
}

export async function handleGenVkButton<T extends FileMapping>(files: T): Promise<Uint8Array> {
    const result = await convertFilesToFilesSer(files)
...
export async function handleVerifyButton<T extends FileMapping>(files: T): Promise<boolean> {
    const result = await convertFilesToFilesSer(files)
    return verify(result["proof"], result["vk"], result["circuitSettings"], result["srs"])
}
